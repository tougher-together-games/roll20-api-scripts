...
`;
		 ...
		 * - The "prompt" key is greedy and captures all remaining arguments as a single string.
		 * 
		 ...
		 * 
		 * @example
		 * // Greedy prompt key captures everything remaining
		 * const subMap = parseChatSubcommands()(moduleSettings)({
		 *     subcommands: ["lang|Common", "prompt|Hello", "how", "are", "you?"]
		 * });
		 * log(subMap);
		 * // Output: { lang: "Common", prompt: "Hello how are you?" }
		 */
		parseChatSubcommands: function () {
			return (moduleSettings) => {
				...
				return ({ subcommands }) => {
					...
					// ANCHOR Const: greedyKeys - Keys that capture all remaining arguments as a single string
					const greedyKeys = ["prompt"];
					...
					for (let i = 0; i < subcommands.length; i++) {
						...
						if (delimiterMatch) {
							...
							// Check if this is a greedy key that captures everything remaining
